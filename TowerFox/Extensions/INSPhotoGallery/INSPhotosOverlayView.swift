...
//        rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .action, target: self, action: #selector(INSPhotosOverlayView.actionButtonTapped(_:)))
    ...
    private func setupCaptionLabel() {
        ...
        if #available(iOS 11.0, *) {
            let bottomConstraint = NSLayoutConstraint(item: safeAreaLayoutGuide, attribute: .top, relatedBy: .equal, toItem: captionLabel, attribute: .top, multiplier: 1.0, constant: -20)
            self.addConstraints([bottomConstraint])
        } else {
            let bottomConstraint = NSLayoutConstraint(item: self, attribute: .top, relatedBy: .equal, toItem: captionLabel, attribute: .top, multiplier: 1.0, constant: -20)
            self.addConstraints([bottomConstraint])
        }
        let leadingConstraint = NSLayoutConstraint(item: captionLabel!, attribute: .leading, relatedBy: .equal, toItem: self, attribute: .leading, multiplier: 1.0, constant: 50.0)
        let trailingConstraint = NSLayoutConstraint(item: captionLabel!, attribute: .trailing, relatedBy: .equal, toItem: self, attribute: .trailing, multiplier: 1.0, constant: -50.0)
