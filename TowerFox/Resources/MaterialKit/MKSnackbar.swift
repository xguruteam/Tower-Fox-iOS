...
public class MKSnackbar: UIControl, MKSnackbarDelegate {
            ...
            UIView.animate(
                ...
                animations: {() -> Void in
                    ...
                    if finished {
                        ...
                        self.performDelegateAction(action: Selector("snackbarDismissed(_ :)"))
    ...
    @objc internal func actionButtonClicked(_ sender: AnyObject) {
        performDelegateAction(action: Selector("actionClicked(_ :)"))
        if let actionButton = actionButton {
            actionButton.isEnabled = false
        }
        dismiss()
        ...
        if let rootView = rootView {
            ...
            UIView.animate(
                ...
                animations: {() -> Void in
                    ...
                    if finished {
                        self.isAnimating = false
                        self.performDelegateAction(action: Selector("snackbarShown(_ :)"))
                        self.perform(
...
class MKSnackbarManager: NSObject, MKSnackbarDelegate {
    ...
    @objc internal func snackbabrDismissed(_ snackbar: MKSnackbar) {
        if var snackbarQueue = snackbarQueue {
            if let index = snackbarQueue.index(where: { $0 == snackbar}){
