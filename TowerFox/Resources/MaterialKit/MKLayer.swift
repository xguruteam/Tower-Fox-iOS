...
public class MKLayer: CALayer, CAAnimationDelegate {
    ...
    private func enableElevation() {
        if let superLayer = self.superLayer {
            ...
            superLayer.shadowOpacity =  self.shadowOpacity
            superLayer.shadowRadius =  self.shadowRadius
            superLayer.shadowColor = self.shadowColor
            superLayer.shadowOffset = self.shadowOffset//CGSize(width: 0, height: 0)
        }
    }
}
